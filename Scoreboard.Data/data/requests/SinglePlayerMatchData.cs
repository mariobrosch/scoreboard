...
namespace Scoreboard.Data.data.requests
{
    public static class SinglePlayerMatchData
    {
        ...
        {
            var data = JsonConvert.SerializeObject(singlePlayerMatch);
            var singlePlayerMatchId = RequestHandler.MakeRequest(HttpMethods.POST, tableName, "", "", data);
            var newSinglePlayerMatch = GetData(singlePlayerMatchId);
        ...
        {
            var singlePlayerMatch = JsonConvert.DeserializeObject<List<SinglePlayerMatch>>(GetData(singlePlayerMatchId)).First();
